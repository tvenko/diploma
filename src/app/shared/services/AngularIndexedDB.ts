...
export class AngularIndexedDB {
  ...
  constructor() {
    ...
    this.dbWrapper = new DbWrapper('myDb', 1);
  }
  ...
  getByKey(storeName: string, key: any) {
    ...
    const promise = new Promise<any>((resolve, reject) => {
      ...
      const transaction = self.dbWrapper.createTransaction({ storeName: storeName,
...
class DbWrapper {
  ...
  createTransaction(options: { storeName: string, dbMode: string, error:
    (e: Event) => any, complete: (e: Event) => any, abort?: (e: Event) => any }): IDBTransaction {
